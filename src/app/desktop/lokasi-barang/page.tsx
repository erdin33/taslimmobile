...
export default function LokasiBarangPage() {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 pb-32 space-y-6 text-foreground">
      ...
      <Card className="border-border/70 bg-card shadow-2xs rounded-2xl p-5 flex flex-col justify-between relative overflow-hidden">
      ...
      {categoryStats.length > 0 && (
        <Card className="border-border/70 bg-card shadow-2xs rounded-2xl p-4 space-y-3">
          <div className="flex items-center justify-between gap-2 border-b border-border/40 pb-2.5">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                className="text-xs text-primary hover:text-primary hover:bg-primary/10 h-7 px-2 font-medium cursor-pointer"
              >
            ...
            {categoryStats.map((cat) => {
              ...
              return (
                <div
                  ...
                  className={`p-3 rounded-xl border ${cat.borderColor} ${cat.bgColor} flex items-center justify-between gap-2 transition-all duration-150 hover:scale-[1.02] hover:shadow-xs cursor-pointer group`}
                  title={`Klik untuk melihat barang ${cat.name}`}
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    <div className={`size-8 rounded-lg bg-background/90 shadow-2xs flex items-center justify-center shrink-0 ${cat.textColor}`}>
                      <CategoryIcon className="size-4" />
                    </div>
                    <div className="min-w-0">
                      <span className="text-xs font-medium text-muted-foreground block truncate leading-tight">
                        {cat.name}
                      </span>
                      <div className="flex items-baseline gap-1 mt-0.5">
                        <span className={`text-base font-extrabold leading-none ${cat.textColor}`}>
                  ...
                  <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-md bg-background/80 text-muted-foreground shrink-0 shadow-2xs border border-border/40">
                    {cat.pct}%
                  </span>
                </div>
              );
      ...
      <Card className="border-border/70 bg-card shadow-2xs rounded-2xl p-5 space-y-3">
        ...
        <div className="grid grid-cols-3 gap-4 pt-1">
          <div
            ...
            className={`border rounded-2xl p-4 text-center transition-all cursor-pointer ${
              filterType === "rak"
      ...
      <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-3">
        ...
        <div className="flex items-center gap-2 flex-1 sm:justify-end">
          ...
          >
            ...
            <span>Tambah {filterType === "rak" ? "Rak" : filterType === "kardus" ? "Kardus" : "Pallet"}</span>
          </Button>
        </div>
      </div>
