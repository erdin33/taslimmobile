...
// Estimasi ukuran byte hasil decode dari data URL base64 (tanpa prefix mime).
...
/**
 * Kompres gambar (data URL) hingga ukuran hasil decode ≤ maxBytes (default 1MB)
 * dengan menurunkan kualitas JPEG bertahap. Menjaga watermark yang sudah ada
 * di data URL karena prosesnya re-encode dari gambar aslinya.
 */
...
const getBaseUrl = () => {
	const baseUrl =
		(import.meta as any).env.URL ||
		(import.meta as any).env.VITE_URL ||
...
export default function TugasHarianPage() {
	...
	const [completedItems, setCompletedItems] = useState<
		Record<string, { imageUrl: string; timestamp: string }>
	>({});
	...
	const [geotag, setGeotag] = useState<{ lat: number; lng: number } | null>(
		null,
	);
	...
	useEffect(() => {
		...
		const fetchFallbackLocation = async () => {
			...
			try {
				...
				console.warn(
					"Fallback IP GPS gagal (tidak ada internet/diblokir)",
					err,
				);
	...
	const drawWatermark = useCallback(
		(sourceCanvas: HTMLCanvasElement, itemForWatermark: BarangUnit): string => {
			...
			if (geotag) {
				if (geotag.lat === 0 && geotag.lng === 0) {
					ctx.fillText(
						`Lokasi: Gagal mendapatkan sinyal`,
						20,
						height - bottomOffset + 80,
					);
				} else {
					ctx.fillText(
						`Lokasi: ${geotag.lat.toFixed(6)}, ${geotag.lng.toFixed(6)}`,
						20,
						height - bottomOffset + 80,
					);
				}
			...
			// Baris 3: SN & User
			ctx.fillText(
				`SN: ${itemForWatermark.serialNumber}`,
				20,
				height - bottomOffset + 120,
			);
			ctx.fillText(
				`User: ${user?.displayName || "Mitra"}`,
				Math.max(20, width - 300),
				height - bottomOffset + 120,
			);
	...
	const handleConfirmPhoto = useCallback(async () => {
		...
		const toastId = toast.loading("Memeriksa Barcode pada foto...", {
			duration: 15000,
		});
		...
		try {
			...
			if ("BarcodeDetector" in window) {
				try {
					...
					console.warn(
						"Native BarcodeDetector fail, fallback to html5-qrcode",
						err,
					);
				}
			}
			...
			if (detectedValues.length === 0) {
				try {
					const scanner = new Html5Qrcode("hidden-recon-scanner", {
						verbose: false,
					});
			...
			const isMatch = detectedValues.some((val) => {
				...
				if (
					targetWithoutPrefix.length >= 5 &&
					rawWithoutPrefix.includes(targetWithoutPrefix)
				)
					return true;
							...
							<div className="absolute inset-0 flex items-center justify-center z-10">
								<div className="text-center text-white p-6 space-y-3 max-w-xs">
									...
									<Button
										...
										onClick={() => activeItem && handleStartScan(activeItem)}>
							...
							<div className="absolute bottom-0 inset-x-0 p-6 pb-[calc(2rem+env(safe-area-inset-bottom,24px))] flex items-center justify-center gap-4 bg-gradient-to-t from-black/80 via-black/40 to-transparent z-20">
								{previewImage ? (
									...
									<div className="flex flex-col items-center gap-2">
										...
										<span className="text-white drop-shadow-md text-sm font-medium">
											Ambil Foto
										</span>
									</div>
								)}
			...
			<div className="flex items-start justify-between mb-2">
				<div className="space-y-1">
					...
					<p className="text-sm text-muted-foreground">
						Ambil foto fisik barang untuk memverifikasi ketersediaan material di
						lapangan.
					</p>
				</div>
				<Button
				...
				<TabsContent value="pending" className="space-y-4">
					{pendingItems.length === 0 ? (
						<div className="text-center py-12 text-muted-foreground border-2 border-dashed rounded-xl">
							...
							<p className="text-sm">
								Tidak ada barang yang perlu di-recon lagi hari ini.
							</p>
						</div>
					) : (
						pendingItems.map((item) => (
							<Card
								key={item.id}
								className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-l-4 border-l-amber-500">
								<div className="space-y-1">
									<div className="flex items-center gap-2">
										...
										<Badge
											variant="outline"
											className="text-[10px] h-5 px-1.5 bg-amber-500/10 text-amber-600 border-amber-200">
											Pending
										</Badge>
									</div>
					...
					{doneItems.length === 0 ? (
						...
						doneItems.map((item) => {
							...
							return (
								<Card
									key={item.id}
									className="p-4 flex flex-col md:flex-row gap-4 border-l-4 border-l-emerald-500 overflow-hidden">
